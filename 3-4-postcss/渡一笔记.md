# PostCss
> PostCss是单独的一个内容，本身与webpack无关
css的工程化存在着诸多问题，而解决这些问题又有各种各样的解决方案。
我们单独看css，光是样式本身就有很多事情需要处理。

将这些事情集中起来统一处理，就是PostCss诞生的理念


PostCss类似一个编译器，可以将样式编译成最终的css代码。

但PostCss和LESS、SASS的思路不同，它其实只做一些代码分析之类的事情，将分析的结果交给插件，具体的代码转换操作是插件去完成的。

> 样式源码 => PostCss（插件1,插件二,插件三） => 编译后的样式代码
